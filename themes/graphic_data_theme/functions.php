...
function enqueue_sanctuary_map_script()
{
	...
	wp_enqueue_script(
		'sanctuary-map',
		get_template_directory_uri() . '/assets/js/sanctuary-map.js',
		array('leaflet-js'),
		get_theme_asset_version(),
		array('strategy' => 'defer')
	);
}
add_action('wp_enqueue_scripts', 'enqueue_sanctuary_map_script');

/**
 * Enqueues Leaflet CSS and JS for the map view.
 * 
 * Only loads on the front page where the map is displayed.
 */
