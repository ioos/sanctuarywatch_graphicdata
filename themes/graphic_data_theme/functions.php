...
{ //maybe add a field named 'modal_scene'
	$args = array(
		'post_type' => 'any',
		'meta_query' => array(
 ...
 * @return array|null The argument array to be used with a WordPress query, or null if invalid.
 */
...
/**
 * Enqueue scripts for the front page map.
 */
function enqueue_front_page_map()
{
	if (is_front_page()) {
		// Enqueue Plotly CDN
		wp_enqueue_script('plotly-cdn', 'https://cdn.plot.ly/plotly-2.27.0.min.js', array(), '2.27.0', true);

		// Enqueue Front Page Map Script
		wp_enqueue_script(
			'front-page-map',
			get_stylesheet_directory_uri() . '/assets/js/front-page-map.js',
			array('plotly-cdn'),
			filemtime(get_stylesheet_directory() . '/assets/js/front-page-map.js'),
			true
		);
	}
}
add_action('wp_enqueue_scripts', 'enqueue_front_page_map');
?>