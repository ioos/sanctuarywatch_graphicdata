...
if ($instances_query->have_posts()) {
	while ($instances_query->have_posts()) {
		...
		// Collect map data
		$lat = get_post_meta($instance_id, 'instance_latitude', true);
		$lon = get_post_meta($instance_id, 'instance_longitude', true);
		$status = get_post_meta($instance_id, 'instance_status', true);

		if ($status === 'Published' && !empty($lat) && !empty($lon)) {
			$legacy = get_post_meta($instance_id, 'instance_legacy_content', true);
			$url = '';

			if ($legacy === 'no') {
				$slug = get_post_meta($instance_id, 'instance_slug', true);
				// $instance_overview_scene already fetched above
				$overview_post = get_post($instance_overview_scene);
				if ($overview_post) {
					$post_name = $overview_post->post_name;
					$url = $instance_slug . "/" . $post_name;
				}
			} else {
				$url = $instance_legacy_content_url;
			}
...
<body>
	...
	<div id="entire_thing">
		...
		// Map Container
		if (!empty($map_instances)) {
			echo '<div class="container-fluid-index main-container" style="margin-bottom: 20px;">';
			echo '<div id="front-page-map" style="width: 100%; height: 500px; border-radius: 8px; overflow: hidden;"></div>';
			echo '</div>';
		}
	...
	<?php
	// Output map data for JS
	echo "<script>var sanctuaryMapData = " . json_encode($map_instances) . ";</script>";
	get_footer();
	?>
