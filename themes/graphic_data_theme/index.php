	...
	<div id="entire_thing">
		...
		$map_args = array(
			...
			'meta_query' => array(
				...
				array(
					'key' => 'instance_latitude',
					'compare' => 'EXISTS',
				),
				array(
					'key' => 'instance_longitude',
					'compare' => 'EXISTS',
				),
			),
		);
		...
		if ($map_query->have_posts()) {
			while ($map_query->have_posts()) {
				...
				$lng = get_post_meta($instance_id, 'instance_longitude', true);
				...
				$title = get_the_title();
				$tile_image = get_post_meta($instance_id, "instance_tile", true);

				$url = '';
				if ($slug && $overview_scene_id) {
					$overview_post = get_post($overview_scene_id);
					if ($overview_post) {
						$url = home_url('/' . $slug . '/' . $overview_post->post_name);
					}
				}

				if (is_numeric($lat) && is_numeric($lng) && !empty($url)) {
					$map_data[] = array(
						'title' => $title,
						'lat' => $lat,
						'lng' => $lng,
		...
		if (!empty($map_data)) {
			echo '<div class="container-fluid-index main-container sanctuary-map-container">';
			echo '<div id="sanctuary-map"></div>';
			echo '</div>';
			echo '<script>window.sanctuaryData = ' . json_encode($map_data) . ';</script>';
		}

		// Sanctuary Map Section
		?>
		<div class="sanctuary-map-container">
			<div class="container-fluid-index main-container">
				<h2 class="sanctuary-map-heading">Sanctuary Locations</h2>
			</div>
			<div class="container-fluid-index main-container">
