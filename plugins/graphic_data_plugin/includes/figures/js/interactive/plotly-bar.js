...
function injectOverlays(
	...
	if (figureArguments.EvaluationPeriod === 'on') {
		const start = figureArguments.EvaluationPeriodStartDate;
		const end = figureArguments.EvaluationPeriodEndDate;
		const startDate = new Date(start).toLocaleDateString();
		const endDate = new Date(end).toLocaleDateString();
		const fillColor =
			(figureArguments.EvaluationPeriodFillColor || '#999') + '15';
	...
	for (let i = 0; i <= Number(figureArguments.EventMarkersField); i++) {
		...
		if (figureArguments.EventMarkers === 'on') {
			const axisType = figureArguments[`EventMarkersEventAxis${i}`];
			const label = figureArguments[`EventMarkersEventText${i}`];
			const color =
				figureArguments[`EventMarkersEventColor${i}`] || '#000';

			if (axisType === 'x') {
				const date = figureArguments[`EventMarkersEventDate${i}`];
				overlays.push({
					x: [date, date],
					y: [yMin, yMax],
					type: 'scatter',
			...
			if (axisType === 'y') {
				const yValue = parseFloat(
					figureArguments[`EventMarkersEventYValue${i}`],
					10
				);
 ...
 */
async function producePlotlyBarFigure(
	targetFigureElement,
	...
	try {
		...
		if (figureID == targetElementpostID) {
			...
			for (let i = 1; i <= figureArguments.NumberOfBars; i++) {
				...
				const StackedSeparatorColor =
					figureArguments[
						targetBarColumn + 'StackedSeparatorLineColor'
					];
				...
				function lightenColor(hex, factor = 0.2) {
					...
					const r = Math.min(
						255,
						Math.floor(((rgb >> 16) & 0xff) + 255 * factor)
					);
					const g = Math.min(
						255,
						Math.floor(((rgb >> 8) & 0xff) + 255 * factor)
					);
					const b = Math.min(
						255,
						Math.floor((rgb & 0xff) + 255 * factor)
					);
					return `rgb(${r},${g},${b})`;
				}
				...
				if (isStacked === 'on' && columnXHeader !== 'None') {
					...
					const values = dataToBePlotted[columnYHeader].map((val) =>
						parseFloat(val)
					);
					const groupMap = {};
					...
					const xValue =
						figureArguments[targetBarColumn + 'Title'] ||
						`Bar ${i}`;
				...
				const showPercentiles =
					figureArguments[targetBarColumn + 'Percentiles'];
				const showMean = figureArguments[targetBarColumn + 'Mean'];
				const showMean_ValuesOpt =
					figureArguments[targetBarColumn + 'MeanField'];
				if (showPercentiles === 'on' || showMean === 'on') {
					...
					if (showPercentiles === 'on') {
						allBarsPlotly.push({
							...
							line: {
								dash: 'dot',
								color:
									figureArguments[targetBarColumn + 'Color'] +
									'60',
							},
					...
					if (showMean_ValuesOpt != 'auto' && showMean === 'on') {
						const ExistingMeanValue = dataToBePlotted[
							showMean_ValuesOpt
						].filter((item) => item !== '');
						const mean =
							ExistingMeanValue.reduce((a, b) => a + b, 0) /
							ExistingMeanValue.length;
						const filteredX = plotlyX.filter((item) => item !== '');
				...
				const errorArrayRaw =
					figureArguments[targetBarColumn + 'ErrorBars'] === 'on'
						? figureArguments[
								targetBarColumn + 'ErrorBarsInputValues'
							] === 'auto'
							? new Array(plotlyY.length).fill(
									computeStandardDeviation(plotlyY)
								)
							: (
									dataToBePlotted[
										figureArguments[
											targetBarColumn +
												'ErrorBarsInputValues'
										]
									] || []
								)
									.map((val) => parseFloat(val))
				...
				const error_y = errorArrayRaw
					? {
							...
							color:
								figureArguments[
									targetBarColumn + 'ErrorBarsColor'
								] || '#000',
							thickness: 1,
							width: 5,
						}
			...
			const layout = {
				...
				yaxis: {
					...
					rangemode: 'tozero',
					autorange:
						figureArguments.YAxisLowBound === '' &&
						figureArguments.YAxisHighBound === ''
							? true
							: false,
					range:
						figureArguments.YAxisLowBound !== '' &&
						figureArguments.YAxisHighBound !== ''
							? [
									parseFloat(figureArguments.YAxisLowBound),
									parseFloat(figureArguments.YAxisHighBound),
								]
							: undefined,
					tickmode: graphTickModeBool,
					ticks: graphTickPositionBool,
			...
			const config = {
				...
				modeBarButtonsToRemove: [
					...
					'hoverCompareCartesian', //'toImage', 'resetScale2d', 'select2d'
				],
			};
...
function loadDefaultInteractiveBarArguments(jsonColumns) {
	...
	if (numberOfBars > 0) {
		...
		for (const key of Object.keys(currentObj)) {
			const isWithinBars = barPrefixes.some((prefix) =>
...
function plotlyBarParameterFields(jsonColumns, interactive_arguments) {
	...
	const featureNames = [
		'Show X&Y Lines on Grid',
		'Remove Outside Graph Ticks',
	];
	...
	axisTitleArray.forEach((axisTitle) => {
		...
		const labelInputAxisTitle = document.createElement('label');
		labelInputAxisTitle.for = axisTitle + 'AxisTitle';
		labelInputAxisTitle.innerHTML = axisTitle + ' Axis Title';
		const inputAxisTitle = document.createElement('input');
		...
		rangeBound.forEach((bound) => {
			...
			const labelBound = document.createElement('label');
			labelBound.for = axisTitle + bound + 'Bound';
			labelBound.innerHTML = axisTitle + ' Axis, ' + bound + ' Bound';
			const inputBound = document.createElement('input');
	...
	for (let i = 1; i < 15; i++) {
		const selectNumberBarsOption = document.createElement('option');
	...
	const labelSelectXAxisFormat = document.createElement('label');
	labelSelectXAxisFormat.for = 'XAxisFormat';
	labelSelectXAxisFormat.innerHTML = 'X Axis Date Format';
	const selectXAxisFormat = document.createElement('select');
	...
	dateFormats.forEach((dateFormat) => {
		const selectXAxisFormatOption = document.createElement('option');
...
function displayBarFields(numBars, jsonColumns, interactive_arguments) {
	...
	if (numBars > 0) {
		...
		labelStackedBarColumns.innerHTML =
			'Group Bars by X Axis (Stacked Columns)';
						...
						function createDropdown(labelText, selectId) {
							...
							if (feature === 'Mean' || feature === 'ErrorBars') {
								...
								if (feature != 'ErrorBars') {
									autoOpt.value = 'auto';
									autoOpt.innerHTML =
										'Auto Calculate Based on Bar Column Selection';
