...
const interactive_line_arguments_value = document.getElementById(
...
document.addEventListener('DOMContentLoaded', function () {
	const row = document
		.getElementById('interactive_line_arguments_editor')
		.closest('tr');
	if (row) {
...
async function loadJson(targetContainer) {
	...
	if (lengthJsonColumns == 0) {
		...
		const iaEl = document.getElementById('interactive_line_arguments_value')
			.dataset.value;
		...
		if (
			...
			interactive_arguments === undefined
		) {
...
function fillFormFieldValues(elementID) {
	...
	try {
		...
		if (interactiveFields != '' && interactiveFields != null) {
			...
			if (
				resultJSON[elementID] != undefined &&
				resultJSON[elementID] != ''
			) {
