...
const jsonBarColumns = {};
let fieldValueBarSaved;

const interactive_bar_arguments_value = document.getElementById(
	'interactive_bar_arguments_editor'
).value;

//Hide the Bar Graph (Time Series) Arguments Editor row on load
document.addEventListener('DOMContentLoaded', function () {
	const row = document
		.getElementById('interactive_bar_arguments_editor')
		.closest('tr');
	if (row) {
...
async function loadBarJson(targetBarContainer) {
	...
	if (lengthJsonColumns == 0) {
		...
		const iaEl = document.getElementById('interactive_bar_arguments_value')
			.dataset.value;
		...
		if (typeof fillFormFieldBarValues === 'function') {
			...
			if (fieldValueBarSaved !== undefined) {
				selectGraphType.value = fieldValueBarSaved;
			}
		}
...
function secondaryGraphBarFields(interactive_arguments) {
	...
	const secondaryGraphDiv = document.getElementById(
		'secondaryGraphBarFields'
	);
...
function clearPreviousGraphFields() {
	const assignBarColumnsToPlot = document.getElementById(
		'assignBarColumnsToPlot'
	);
...
function logFormFieldBarValues() {
	...
	document.getElementById('interactive_bar_arguments_editor').value =
...
function fillFormFieldBarValues(elementID) {
	...
	const interactiveBarFields = document.getElementById(
		'interactive_bar_arguments_editor'
	).value;
...
function plotlyBarParameterFields(jsonBarColumns, interactive_arguments) {
	...
	fieldValueBarSaved = fillFormFieldBarValues(
	...
	if (numBars > 0) {
		...
		fieldLabels.forEach((fieldLabel) => {
			...
			if (fieldLabel[0] != 'XAxis') {
				...
				if (fieldLabelNumber % 2 != 0) {
					barTypeRow.classList.add('fieldBackgroundColor');
				}
				...
				for (let i = 0; i < features.length; i++) {
					...
					if (['ErrorBars', 'Stacked'].includes(feature)) {
						...
						function createColorfield(labelText, inputId) {
							...
							const saved = fillFormFieldBarValues(
								input.id,
								interactive_arguments
							);
							if (saved) {
...
function startLoadBarJson() {
	const targetBar = document.getElementById('barDefaultSelector');
