 ...
 */
async function render_tab_info(
	tabContentElement,
	tabContentContainer,
	info_obj,
	idx
	...
	if (info_obj.scienceText != '') {
		...
		const icon1 = `<i class="fa fa-clipboard-list" role="presentation" aria-label="clipboard-list icon" style=""></i> `;
	...
	if (info_obj.dataLink != '') {
		...
		const icon2 = `<i class="fa fa-database" role="presentation" aria-label="database icon"></i>`;
		...
		case 'Internal':
			...
			if (info_obj.externalAlt) {
				...
				const restURL =
					...
					'//' +
					host +
					'/wp-json/graphics_data/v1/media/alt-text-by-url?image_url=' +
					encodeURI(img.src);
				console.log(restURL);
				fetch(restURL)
	...
	figureDiv.setAttribute(
		'style',
		'width: 100% !important; height: auto; display: block; margin: 0; margin-top: 2%'
	);
	...
	const longCaption = document.createElement('p');
	...
	switch (figureType) {
		...
		case 'Interactive':
			img.setAttribute(
				'style',
				'width: 100% !important; height: auto; display: flex; margin: 0; margin-top: 2%'
			);

			const plotDiv = document.querySelector(`#plotlyFigure${postID}`);
			try {
				plotDiv.style.width = '100%';
			} catch {}
			break;
