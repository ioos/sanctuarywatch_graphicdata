 ...
 */
class Graphic_Data_GitHub_Updater {
	/**
	...
	private function get_repository_info() {
		...
		if ( is_null( $this->github_response ) ) {
			...
			if ( ! is_wp_error( $releases_response ) ) {
				...
				if ( 403 === $response_code && '0' === $rate_limit_remaining ) {
					...
					if ( is_array( $commit_data ) && isset( $commit_data['sha'] ) ) {
						...
						$this->github_response = array(
							...
							'published_at' => isset( $commit_data['commit']['author']['date'] ) ?
								wp_date( 'Y-m-d H:i:s', strtotime( $commit_data['commit']['author']['date'] ) ) :
								wp_date( 'Y-m-d H:i:s' ),
	...
	private function recursive_remove_directory( $directory ) {
		...
		if ( empty( $directory ) || '/' == $directory || ABSPATH == $directory ) {
			return false;
		}
