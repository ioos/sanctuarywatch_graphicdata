...
document
	.querySelector('[data-depend-id="instance_footer_columns"]')
	.addEventListener('change', displayFooterEntries);
document
	.querySelector('[data-depend-id="instance_footer_columns"]')
	.nextElementSibling.addEventListener('change', displayFooterEntries);
...
document
	.querySelector('select[name="instance_legacy_content"]')
	.addEventListener('change', displayLegacyContentField);

// Show relevant footer entries
function displayFooterEntries() {
	const footerColumnNumber = document.getElementsByName(
		'instance_footer_columns'
	)[0].value;
	for (let i = 3; i > footerColumnNumber; i--) {
		const target_text_title =
			'instance_footer_column' +
			i +
			'[instance_footer_column_title' +
			i +
			']';
		const target_text_title_div =
			document.getElementsByName(target_text_title)[0];
		target_text_title_div.value = '';
		const target_text_content =
			'instance_footer_column' +
			i +
			'[instance_footer_column_content' +
			i +
			']';
		const target_text_content_div =
			document.getElementsByName(target_text_content)[0];
		...
		const visualEditor = trumbowygBox
			? trumbowygBox.querySelector(
					'.trumbowyg-editor[contenteditable="true"]'
				)
			: null;
		...
		target_text_title_div.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.style.display =
			'none';
	}
	...
	if (onCorrectEditPage('instance') == true) {
		if (cookieExists('instance_error_all_fields')) {
			...
			const instanceFieldNames = [
				...
				'instance_legacy_content',
				'instance_legacy_content_url',
				'instance_footer_column_title_1',
				'instance_footer_column_title_2',
				'instance_footer_column_title_3',
			];

			instanceFieldNames.forEach((element) => {
				document.getElementsByName(element)[0].value =
					instanceCookieValues[element];
			});

			// Fill in values for complex fieldsets
			document.getElementsByName(
				'instance_footer[instance_footer_about]'
			)[0].value = instanceCookieValues.instance_footer_about;
			document.getElementsByName(
				'instance_footer[instance_footer_contact]'
			)[0].value = instanceCookieValues.instance_footer_contact;
			document.getElementsByName(
				'instance_footer[instance_footer_reports]'
			)[0].value = instanceCookieValues.instance_footer_reports;
		}
	}
}
