 ...
 * explicit choice.
 *
 * Targeted fields are supplied by PHP via `wp_localize_script` as
 * `graphicDataNewTab.targetFields` (an array of TinyMCE editor IDs).
 *
...
( function () {
	...
	/**
	 * Configuration object injected by PHP via wp_localize_script.
	 *
	...
	/**
	 * Safely read the localised config. If the object is missing the script
	 * exits silently so it cannot break unrelated admin pages.
	 *
	 * @type {GraphicDataNewTabConfig|null}
	 */
	var config = ( typeof graphicDataNewTab !== 'undefined' ) ? graphicDataNewTab : null;
	 ...
	 * @type {string[]}
	 */
	var targetFields = Array.isArray( config.targetFields ) ? config.targetFields : [];
	...
	function attachModalCheckboxHook() {
		...
		wpLink.open = function () {
			...
			var editorId = getActiveEditorId();
			...
			if ( editorId && isTargetField( editorId ) ) {
				...
				setTimeout( function () {
					var checkbox = document.getElementById( 'wp-link-target' );
	...
	function init() {
		...
		var interval = setInterval( function () {
			...
			if ( typeof wpLink === 'undefined' ) {
				var wpLinkAttempts  = 0;
				var wpLinkMaxChecks = 50;
				var wpLinkInterval  = setInterval( function () {
					wpLinkAttempts++;
					attachModalCheckboxHook();
					if (
