...
(function() {
    ...
    var originalEdit = window.inlineEditPost ? window.inlineEditPost.edit : null;
    ...
    window.inlineEditPost.edit = function(id) {
        ...
        if (postId > 0) {
            ...
            var slugInput = editRow.querySelector('input[name="post_name"]');
            var noticeContainer = editRow.querySelector('.inline-edit-col');
            
            if (!slugInput || !noticeContainer) return;
            
            // Remove any existing notice
            removeNotices(editRow);
            
            // Remove previous event listener if exists
            var newSlugInput = slugInput.cloneNode(true);
            ...
            slugInput.addEventListener('blur', function() {
                ...
                var checkingNotice = createNotice('info', sceneQuickEdit.messages.checking);
                noticeContainer.insertBefore(checkingNotice, noticeContainer.firstChild);
                ...
                .then(function(response) {
                    ...
                    if (checkingNotice.parentNode) {
                        checkingNotice.parentNode.removeChild(checkingNotice);
                    }
                    
                    if (response.success && response.data.exists) {
                        // Show warning
                        var warningNotice = createNotice('warning', '<strong>⚠️ ' + response.data.message + '</strong>');
                        noticeContainer.insertBefore(warningNotice, noticeContainer.firstChild);
                    }
                })
    ...
    function createNotice(type, message) {
        ...
        notice.className = 'scene-slug-notice notice notice-' + type + ' inline';
    ...
    function removeNotices(editRow) {
        var notices = editRow.querySelectorAll('.scene-slug-notice');
    ...
    document.addEventListener('click', function(e) {
        if (e.target.classList.contains('cancel') || e.target.closest('.cancel')) {
            var notices = document.querySelectorAll('.scene-slug-notice');
            notices.forEach(function(notice) {
                if (notice.parentNode) {
                    notice.parentNode.removeChild(notice);
                }
