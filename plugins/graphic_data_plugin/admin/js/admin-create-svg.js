document.addEventListener('DOMContentLoaded', function () {
	...
	rangeIconNumber.addEventListener('change', function () {
		...
		formSection.parentElement.insertBefore(
			fieldsContainer,
	...
	function showError(message) {
		...
		errorDiv.className =
			'svgValidationError notice notice-error is-dismissible';
		errorDiv.innerHTML =
	...
	function validateForm() {
		...
		const existingError =
			document.getElementsByClassName('svgValidationError');
		...
		const iconNumber = parseInt(
			document.getElementById('svgIconNumber').value
		);
		...
		if (iconNumber > 0) {
			...
			for (let i = 1; i <= iconNumber; i++) {
				...
				if (/\s/.test(labelValue)) {
					validationErrors.push(
						'Icon ' + i + ' label cannot contain spaces.'
					);
				}
			...
			const labelValues = iconLabels.map(function (item) {
				return item.value;
			});
			...
			if (uniqueLabels.size !== labelValues.length) {
				// Find duplicates
				const duplicates = labelValues.filter(
		...
		.addEventListener('click', function () {
			...
			fetch(
				ajaxurl.replace('admin-ajax.php', '') +
					'../wp-content/plugins/graphic_data_plugin/admin/images/create_svg_template_illustrator.svg'
			)
				...
				.then((templateContent) => {
					...
					const titleGroup = svgDoc.getElementById('title');
					if (titleGroup) {
						const textElement =
							titleGroup.querySelector('#title-text');
						const rectElement = titleGroup.querySelector(
						...
						if (textElement) {
							...
							const titleText =
								document.getElementById('svgTitle').value;
							...
							const fontSize = 24.48;
							const charWidth = fontSize * 0.6; // Approximate character width for bold Arial
							const textWidth = titleText.length * charWidth;

							// Add padding on both sides
							const padding = 40;
							const newRectWidth = textWidth + padding * 2;
					...
					for (let q = 1; q <= svgIconNumber; q++) {
						for (let k = 0; k < 2; k++) {
							...
							if (targetElement) {
								...
								tspanElements =
									targetElementLabel.getElementsByTagName(
										'tspan'
									);
								Array.from(tspanElements).forEach(
					...
					let filename = prompt(
						'Enter filename for SVG (without extension):',
						defaultFilename
					);
				...
				.catch((error) => {
					...
					alert(
						'Error loading SVG template. Please check the console for details.'
					);
				});
		});
